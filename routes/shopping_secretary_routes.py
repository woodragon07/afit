...
bp = Blueprint('shopping_secretary', __name__)

@bp.route('/shopping_secretary', methods=['POST'])
def shopping_secretary_route():
    """
    쇼핑 도우미 모드:
    ...
    try:
        ...
        if not items:
            return jsonify([{"response": "필요한 품목을 파악하지 못했습니다."}])
        ...
        print("쇼핑비서 모드 오류:", e)
        return jsonify([{"response": "처리 중 오류가 발생했습니다."}])
